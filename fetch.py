...
def _fetch_recent_tw(ticker, mydb, mycursor):  
    mycursor = mydb.cursor()
    sql = "INSERT INTO TW_stock_data VALUES (%s, %s, %s, %s, %s, %s, %s)"
    dic = {}
    ...
    for i in range(len(symb.index)):
        temp = symb.index[i].strftime('%Y-%m-%d')
        query = "SELECT * FROM TW_stock_data WHERE ticker='"+ticker+"' AND day='"+temp+"'"
        mycursor.execute(query)
        result = mycursor.fetchall()
        if not result :
            val = ( ticker, temp, symb['Open'][i].item(), symb['High'][i].item(), symb['Low'][i].item(), symb['Close'][i].item(), symb['Volume'][i].item() )
